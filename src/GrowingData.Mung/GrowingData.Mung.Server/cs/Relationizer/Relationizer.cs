...
namespace GrowingData.Mung.Server {
	...
	/// </summary>
	public class Relationizer : IDisposable {
		...
		public Relationizer(string basePath) {
			_basePath = basePath;
		...
		public void Write(MungServerEvent evt) {
			...
			lock (_syncRoot) {
				foreach (var item in JsonReader.ProcessJsonObject(evt.Token, evt.Type, null)) {

					if (!_writers.ContainsKey(item.Name)) {
						_writers[item.Name] = new RelationalEventWriter(_basePath, item.Name);
					}
